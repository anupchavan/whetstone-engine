...
- ONE notation per symbol, item-wide: pick backticks or math for each name/expression at first use and keep it in EVERY later mention across stem, options, and solution. Program-level expressions — bitwise ops, assignments, loops (`n = n & (n-1)`, `x <<= 1`) — are code in backticks, never math and never LaTeX arrows/operators (\leftarrow, \&, \,). Math delimiters are reserved for genuine mathematics ($O(n \log n)$, $2^{32}$). Mixed forms like a code expression dressed with LaTeX spacing are malformed in BOTH notations and shred rendering.
